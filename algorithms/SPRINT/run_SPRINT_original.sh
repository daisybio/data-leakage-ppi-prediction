...
do
  ...
  bin/predict_interactions -p ../../Datasets_PPIs/SwissProt/yeast_swissprot_oneliner.fasta -h HSP/pre_computed_yeast_HSP -tr data/original/${DATASET}_train_pos.txt -pos data/original/${DATASET}_test_pos.txt -neg data/original/${DATASET}_test_neg.txt -o results/original/${DATASET}_results.txt
done
