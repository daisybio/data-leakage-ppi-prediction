...
def usage():
    print("Usage: {} [train_set OR load_weights + test_set] <OPTIONS>\nEnter {} -h to have the list of optional arguments".format(sys.argv[0],sys.argv[0]))
    sys.exit(1)
...
def make_parser():
    ...
    parser.add_argument('-optimizer', type=str, default='adam', help='Choose among: sgd, rmsprop, adagrad, adadelta, adam, adamax, nadam [default: adam]')
    parser.add_argument('-lr', type=float, default=0.001, help='Learning rate [default: 0.001]')
    parser.add_argument('-gpu', type=int, default=0, help='If you have several GPUs, which one to use [default: 0]')
    parser.add_argument('-nb_gpu', type=int, default=1, help='Number of GPU devices to use. Incompatible with the -gpu option [default: 1]')
    parser.add_argument('-save', type=str2bool, nargs='?', const=True, default=False, help='To save weights of your model')
    parser.add_argument('-tensorboard', type=str2bool, nargs='?', const=True, default=False, help='Save logs for TensorBoard')
    ...
    parser.add_argument('-load', type=str, help='File containing weights to load. You must also give a test set with this option.')
    parser.add_argument('-name', type=str, help='Name complement to produced files, written at the end of the name file.')
    return parser
...
if __name__ == '__main__':
    ...
    file_name = model_name + '_' + now.strftime("%Y-%m-%d_%H:%M") + '_gpu-' + str(args.gpu) + '-' + str(number_gpu) + '_' + optimizer_name + '_' + str(lr) + '_' + str(batch_size) + '_' + str(epochs)
    if name_complement:
    ...
    if not load_weights:
        ...
        if tensorboard:
            callbacks_list.append( callbacks.TensorBoard(log_dir='./tensorboard_logs', histogram_freq=10, batch_size=batch_size, write_graph=False, write_grads=True, write_images=True, embeddings_freq=0, embeddings_layer_names=None, embeddings_metadata=None, embeddings_data=None, update_freq='epoch') )
    ...
    # String of arguments
    arguments = model_name + ', epochs=' + str(epochs) + ', batch=' + str(batch_size) + ', optimizer=' + optimizer_name + ', learning rate=' + str(lr) + ', patience=' + str(patience)

    result_file = open('results/' + file_name + '.txt', 'w')
    ...
    if not test_set:
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig('results/' + file_name + '_loss.png', bbox_inches='tight')
        ...
        plt.ylabel('Accuracy')
        plt.legend()
        plt.savefig('results/' + file_name + '_acc.png', bbox_inches='tight')
        ...
        result_file.write('Validation precision: {}\nValidation recall: {}\nValidation F1-score: {}\n\n'.format(precision, recall, f1_score))
        ...
        result_file.write('Test precision: {}\nTest recall: {}\nTest F1-score: {}\n\n'.format(precision, recall, f1_score))
