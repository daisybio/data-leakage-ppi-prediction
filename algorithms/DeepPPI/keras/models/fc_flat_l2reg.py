...
class FC_Flat_l2reg(AbstractModel):
    def __init__(self):
        super().__init__()

    def get_model(self):
        flat = layers.Flatten()
        first_layer = layers.Dense(1000, kernel_regularizer=regularizers.l2(0.001), activation='relu')
        ...
        second_layer = layers.Dense(1000, kernel_regularizer=regularizers.l2(0.001), activation='relu')
        ...
        # head = layers.Dropout(0.5)(head)
        # head = layers.BatchNormalization()(head)
        head = layers.Dense(100, kernel_regularizer=regularizers.l2(0.001), activation='relu')(head)
        head = layers.Dropout(0.5)(head)
        head = layers.BatchNormalization()(head)
