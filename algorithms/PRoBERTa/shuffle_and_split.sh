#!/bin/bash
# Shuffle and split up family classification or PPI input
INPUT="$1"
OUTPUT="$2"
PREFIX="$3"
...
for SHARD in $(seq $TEST_START $TEST_END); do
    cat "$OUTPUT/$PREFIX.temp$SHARD" >> "$OUTPUT/$PREFIX.split.test$TEST_PCT"
done
...
for SHARD in $(seq $VALID_START $VALID_END); do
    cat "$OUTPUT/$PREFIX.temp$SHARD" >> "$OUTPUT/$PREFIX.split.valid$VALID_PCT"
done
...
for SHARD in $(seq $TRAIN_START $TRAIN_END); do
    cat "$OUTPUT/$PREFIX.temp$SHARD" >> "$OUTPUT/$PREFIX.split.train$TRAIN_PCT"
done
