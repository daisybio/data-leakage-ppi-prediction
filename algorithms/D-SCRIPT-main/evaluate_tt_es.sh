...
declare -A bestepochs=( ["huang"]="07" ["guo"]="07" ["du"]="03" ["pan"]="02" ["richoux_regular"]="01" ["richoux_strict"]="05" ["dscript"]="01" )
...
do
	...
	if [ "$DATASET" == "gold" ]; then
                MODEL="./models/tt_${DATASET}_epoch${EPOCH}.sav"
        else
                MODEL="./models/${DATASET}_tt_original_epoch${EPOCH}.sav"
        fi
	echo evaluating on model $MODEL

	dscript evaluate --test data/original/${DATASET}_test.txt --embedding $EMBEDDING --model $MODEL -o ./results_topsyturvy/original/${DATASET}_es.txt
done
