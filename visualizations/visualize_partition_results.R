...
# custom
custom_results <- lapply(paste0(custom_res,  list.files(custom_res, pattern = '^(du|guo|huang|pan|richoux|dscript)')), fread)
file_names <- tstrsplit(list.files(custom_res, pattern = '^(du|guo|huang|pan|richoux|dscript)'), '.csv', keep=1)[[1]]
...
if(measure == 'Recall'){
  ...
  if(measure == 'Accuracy'){
    wide_df <- dcast(custom_results[dataset=='dscript', ], filename + dataset + encoding + model + train + test ~ V1, value.var='V2')
    ...
    wide_df <- dcast(degree_results[dataset=='dscript', ], filename + partition + dataset + train + test + model ~ V1, value.var='V2')
    wide_df[, Balanced_Accuracy := 0.5 * (Sensitivity + Specificity)]
    wide_df <- wide_df[order(filename)]
    degree_results <- degree_results[order(filename)]
    degree_results[dataset=='dscript' & V1 == 'Accuracy', V2 := wide_df$Balanced_Accuracy]
  }
...
# deepFE
deepFE_results <- lapply(paste0(deepFE_res, list.files(deepFE_res, pattern = '^partition_scores_(du|guo|huang|pan|richoux|dscript)_(both|0)_(0|1).csv', recursive = TRUE)), fread)
file_names <- tstrsplit(list.files(deepFE_res, pattern = '^partition_scores_(du|guo|huang|pan|richoux|dscript)_(both|0)_(0|1).csv', recursive = TRUE), '/', keep=2)[[1]]
...
if(measure == 'Accuracy'){
  wide_df <- dcast(deepFE_results[dataset=='dscript', ], filename + dataset + train + test ~ V1, value.var='Score')
  wide_df[, Balanced_Accuracy := 0.5 * (Recall + Specificity)]
  wide_df <- wide_df[order(filename)]
  deepFE_results <- deepFE_results[order(filename)]
  deepFE_results[dataset=='dscript' & V1 == 'Accuracy', Score := wide_df$Balanced_Accuracy]
}
...
# deepPPI
deepPPI_results <- lapply(paste0(deepPPI_res, list.files(deepPPI_res, pattern='partition_(du|guo|huang|pan|richoux|dscript).*.csv')), fread)
file_names <- tstrsplit(list.files(deepPPI_res, pattern='partition_(du|guo|huang|pan|richoux|dscript).*.csv'), '.csv', keep=1)[[1]]
...
if(measure == 'Accuracy'){
  wide_df <- dcast(deepPPI_results[Dataset=='dscript', ], filename + Model + Dataset + Train + Test + Partition ~ variable, value.var='value')
  wide_df[, Balanced_Accuracy := 0.5 * (Recall + Specificity)]
  wide_df <- wide_df[order(filename)]
  deepPPI_results <- deepPPI_results[order(filename)]
  deepPPI_results[Dataset=='dscript' & variable == 'Accuracy', value := wide_df$Balanced_Accuracy]
}
...
# PIPR
pipr_results <- lapply(paste0(seqppi_res, list.files(seqppi_res, pattern='^partition_(du|guo|huang|pan|richoux|dscript)_(both|0)_(0|1).csv')), fread)
file_names <- tstrsplit(list.files(seqppi_res, pattern='^partition_(du|guo|huang|pan|richoux|dscript)_(both|0)_(0|1).csv'), '.csv', keep=1)[[1]]
...
if(measure == 'Accuracy'){
  wide_df <- dcast(pipr_results[Dataset=='dscript', ], filename + Dataset + Train + Test ~ V1, value.var='Score')
  wide_df[, Balanced_Accuracy := 0.5 * (Recall + Specificity)]
  wide_df <- wide_df[order(filename)]
  pipr_results <- pipr_results[order(filename)]
  pipr_results[Dataset=='dscript' & V1 == 'Accuracy', Score := wide_df$Balanced_Accuracy]
}
...
if(measure == 'Accuracy'){
  wide_df <- dcast(dscript_results[Dataset=='dscript', ], Model + Dataset + Split ~ Metric, value.var='Value')
  wide_df[, Balanced_Accuracy := 0.5 * (Recall + Specificity)]
  wide_df <- wide_df[order(Model, Split)]
  dscript_results <- dscript_results[order(Model, Split)]
  dscript_results[Dataset=='dscript' & Metric == 'Accuracy', Value := wide_df$Balanced_Accuracy]
}
...
if(measure == 'Accuracy'){
  wide_df <- dcast(tt_results[Dataset=='dscript', ], Model + Dataset + Split ~ Metric, value.var='Value')
  wide_df[, Balanced_Accuracy := 0.5 * (Recall + Specificity)]
  wide_df <- wide_df[order(Model, Split)]
  tt_results <- tt_results[order(Model, Split)]
  tt_results[Dataset=='dscript' & Metric == 'Accuracy', Value := wide_df$Balanced_Accuracy]
}
