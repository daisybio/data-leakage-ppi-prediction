...
if (measure == 'Recall') {
  ...
  if (measure == 'Accuracy') {
    ...
    wide_df <-
      dcast(degree_results[dataset == 'dscript',],
...
if (measure == 'Accuracy') {
  wide_df <-
    dcast(pipr_results[Dataset == 'dscript',], filename + Dataset + Train + Test ~ V1, value.var =
            'Score')
  wide_df[, Balanced_Accuracy := 0.5 * (Recall + Specificity)]
  wide_df <- wide_df[order(filename)]
...
if(es){
  fwrite(all_results, file = paste0('results/partition_', measure, '_es.csv'))
}else{
