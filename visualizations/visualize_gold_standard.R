...
if(es){
  deepFE_results <-
    lapply(paste0(
      deepFE_res,
      list.files(deepFE_res, pattern = '^gold.*csv', recursive = TRUE)
    ), fread)
}else{
  deepFE_results <-
    lapply(paste0(
...
file_names <-
  tstrsplit(list.files(deepPPI_res, pattern = 'gold.*.csv'), '.csv', keep =
              1)[[1]]
...
# PIPR
pipr_results <-
  lapply(paste0(seqppi_res, list.files(seqppi_res, pattern = '^gold.*.csv')), fread)
