...
for (measure in c('Accuracy',
  ...
  deepFE_res <-
    '../algorithms/DeepFE-PPI/result/custom/gold_standard_test/'
  ...
  # custom
  custom_results <-
    lapply(paste0(custom_res,  list.files(custom_res, pattern = '^gold.*.csv')), fread)
  file_names <-
    tstrsplit(list.files(custom_res, pattern = '^gold.*.csv'),
              '.csv',
              keep =
                1)[[1]]
  ...
  if (es) {
    deepFE_results <-
      lapply(paste0(
        deepFE_res,
        list.files(deepFE_res, pattern = '^gold.*_es.csv', recursive = TRUE)
      ), fread)
  } else{
    deepFE_results <-
      lapply(paste0(
        deepFE_res,
        list.files(deepFE_res, pattern = '^gold.*_test.csv', recursive = TRUE)
      ), fread)
  }
  ...
  if (es) {
    deepPPI_results <-
      lapply(paste0(
        deepPPI_res,
        list.files(deepPPI_res, pattern = 'gold.*es.csv')
      ), fread)
    file_names <-
      tstrsplit(list.files(deepPPI_res, pattern = 'gold.*standard.csv'),
                '.csv',
                keep =
                  1)[[1]]
  } else{
    deepPPI_results <-
      lapply(paste0(
        deepPPI_res,
        list.files(deepPPI_res, pattern = 'gold_standard.csv')
      ), fread)
    file_names <-
  ...
  if (es) {
    pipr_results <-
      lapply(paste0(
        seqppi_res,
        list.files(seqppi_res, pattern = '^gold.*es.csv')
      ), fread)
    ...
    pipr_results <-
      lapply(paste0(
        seqppi_res,
        list.files(seqppi_res, pattern = '^gold.*standard.csv')
      ), fread)
    names(pipr_results) <- 'gold'
  }
  ...
  pipr_results <-
    pipr_results[, c('Test', 'Dataset') := tstrsplit(Filename, '_')]
  ...
  if (es) {
    fwrite(all_results,
           file = paste0('results/gold_standard_', measure, '_es.csv'))
  } else{
    fwrite(all_results,
           file = paste0('results/gold_standard_', measure, '.csv'))
  }
}