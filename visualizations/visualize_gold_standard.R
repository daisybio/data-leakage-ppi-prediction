...
file_names <-
  tstrsplit(list.files(custom_res, pattern = '^gold.*.csv'), '.csv', keep =
              1)[[1]]
...
if (es) {
  pipr_results <-
    lapply(paste0(seqppi_res, 
                  list.files(seqppi_res, pattern = '^gold.*es.csv')
                  ), fread)
  names(pipr_results) <- 'gold'
} else{
  pipr_results <-
    lapply(paste0(seqppi_res, list.files(seqppi_res, pattern = '^gold.*standard.csv')), fread)
  names(pipr_results) <- 'gold'
}
...
if(es){
  fwrite(all_results, file = paste0('results/gold_standard_', measure, '_es.csv'))
}else{
