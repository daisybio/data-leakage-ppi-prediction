...
# custom
custom_results <- lapply(paste0(custom_res,  list.files(custom_res, pattern='^original_(du|guo|huang|pan|richoux|dscript).*.csv')), fread)
file_names <- tstrsplit(list.files(custom_res, pattern='^original_(du|guo|huang|pan|richoux|dscript).*.csv'), '.csv', keep=1)[[1]]
...
if(measure == 'Recall'){
  ...
  if(measure == 'Accuracy'){
    wide_df <- dcast(custom_results[dataset=='dscript', ], filename + dataset + encoding + method + Model ~ V1, value.var='V2')
...
# deepFE
deepFE_results <- lapply(paste0(deepFE_res, list.files(deepFE_res, pattern = '^original_scores_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript).csv', recursive = TRUE)), fread)
file_names <- tstrsplit(list.files(deepFE_res, pattern = '^original_scores_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript).csv', recursive = TRUE), '/', keep = 1)[[1]]
file_names[grepl('richoux', file_names, fixed=TRUE)] <- gsub('richoux_*', 'richoux-', file_names[grepl('richoux', file_names, fixed=TRUE)])
...
# deepPPI
deepPPI_results <- lapply(paste0(deepPPI_res, list.files(deepPPI_res, pattern='original_(du|guo|huang|pan|richoux|dscript).*.csv')), fread)
file_names <- tstrsplit(list.files(deepPPI_res, pattern='original_(du|guo|huang|pan|richoux|dscript).*.csv'), '.csv', keep=1)[[1]]
file_names[grepl('richoux', file_names, fixed=TRUE)] <- gsub('richoux_*', 'richoux-', file_names[grepl('richoux', file_names, fixed=TRUE)])
...
# PIPR
pipr_results <- lapply(paste0(seqppi_res, list.files(seqppi_res, pattern='^original_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript).csv')), fread)
file_names <- tstrsplit(list.files(seqppi_res, pattern='^original_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript).csv'), '.csv', keep=1)[[1]]
file_names[grepl('richoux', file_names, fixed=TRUE)] <- gsub('richoux_*', 'richoux-', file_names[grepl('richoux', file_names, fixed=TRUE)])
