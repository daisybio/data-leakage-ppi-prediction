...
for (measure in c('Precision',
  ...
  if (es) {
    original_results_es <-
      fread(paste0('results/original_', measure, '_es.csv'))
    gold_standard_results_es <-
      fread(paste0('results/gold_standard_', measure, '_es.csv'))
    original_results_es <-
      rbind(original_results_es, gold_standard_results_es)
    original_results_es$Test <- 'Original'
    rewired_results_es <-
      fread(paste0('results/rewired_', measure, '_es.csv'))
    rewired_results_es$Test <- 'Rewired'
    partition_results_es <-
      fread(paste0('results/partition_', measure, '_es.csv'))
    colnames(partition_results_es) <-
      c('Model', 'Dataset', measure, 'Test')
  }
  original_results <-
    fread(paste0('results/original_', measure, '.csv'))
  gold_standard_results <-
    fread(paste0('results/gold_standard_', measure, '.csv'))
  original_results <- rbind(original_results, gold_standard_results)
  original_results$Test <- 'Original'
  rewired_results <-
    fread(paste0('results/rewired_', measure, '.csv'))
  rewired_results$Test <- 'Rewired'
  partition_results <-
    fread(paste0('results/partition_', measure, '.csv'))
  
  colnames(partition_results) <-
    c('Model', 'Dataset', measure, 'Test')
  ...
  modify_df <- function(df) {
    ...
    df[, Test := factor(Test,
                        levels = c('Original', 'Rewired', 'both->0', 'both->1', '0->1'))]
  ...
  make_result_mat <- function(df) {
    result_mat <-
      as.matrix(dcast(df, Model ~ Dataset + Test, value.var = measure))
    rownames(result_mat) <- result_mat[, 'Model']
    result_mat <- result_mat[,-1]
    class(result_mat) <- 'numeric'
    colnames(result_mat)[colnames(result_mat) == 'gold_standard_Original'] <-
      'Gold_Original'
    annotation_col <-
      as.data.frame(tstrsplit(colnames(result_mat), '_', keep = 2), col.names = c('Test'))
    ...
    annotation_col$Dataset <-
      tstrsplit(colnames(result_mat), '_', keep = 1)[[1]]
    annotation_col$Dataset <- factor(
      annotation_col$Dataset,
      levels = c(
        'Gold',
        'huang',
    ...
    annotation_col <-
      annotation_col[order(annotation_col$Test, annotation_col$Dataset), , drop = FALSE]
  ...
  get_sizes <- function(directory) {
    sprint_data_dir <-
      paste0('../algorithms/SPRINT/data/', directory, '/')
    training_files_pos <-
      list.files(path = sprint_data_dir, pattern = 'train_pos')
    training_files_neg <-
      list.files(path = sprint_data_dir, pattern = 'train_neg')
    train_sizes_pos <-
      sapply(paste0(sprint_data_dir, training_files_pos), function(x) {
        as.integer(system2(
          "wc",
          args = c("-l",
                   x,
    ...
    train_sizes_neg <-
      sapply(paste0(sprint_data_dir, training_files_neg), function(x) {
        as.integer(system2(
          "wc",
          args = c("-l",
                   x,
    ...
    training_files_pos[grepl('richoux', training_files_pos, fixed = TRUE)] <-
      gsub('richoux_*', 'richoux-', training_files_pos[grepl('richoux', training_files_pos, fixed =
                                                               TRUE)])
    names(train_sizes) <-
      tstrsplit(training_files_pos, '_', keep = 1)[[1]]
  ...
  original_sizes <-
    c(original_sizes, c('gold' = prettyNum(as.integer(
      system2(
        "wc",
        args = c(
          "-l",
          '../Datasets_PPIs/Hippiev2.3/Intra0_Intra1_pos_rr.txt',
          " | awk '{print $1}'"
        ),
        stdout = TRUE
      )
  ...
  get_sizes_dscript <- function(directory) {
    dscript_data_dir <-
      paste0('../algorithms/D-SCRIPT-main/data/', directory, '/')
    training_files <-
      list.files(path = dscript_data_dir, pattern = 'train')
    train_sizes <-
      sapply(paste0(dscript_data_dir, training_files), function(x) {
        as.integer(system2(
          "wc",
          args = c("-l",
                   x,
  ...
  gold_size <- prettyNum(as.integer(system2(
    "wc",
    args = c(
      "-l",
  ...
  filenames <-
    tstrsplit(tstrsplit(training_files, '.txt', keep = 1)[[1]], '_', keep =
                c(1, 3))
  names(partition_dscript_sizes) <-
    paste(filenames[[1]], filenames[[2]])
  partition_dscript_sizes <-
    prettyNum(partition_dscript_sizes, big.mark = ',')
  ...
  sprint_label <-
    ifelse(measure == 'AUC', 'SPRINT (AUC)', 'SPRINT (AUPR)')
  pheatmap(
    mat,
    ...
    annotation_colors = list(
      Test = c(
        'Original' = '#AA4499',
        'Rewired' = '#DDCC77',
        'Inter->Intra-1' = '#888888',
        'Inter->Intra-0' = '#44AA99',
        'Intra-0->Intra-1' = '#661100'
      )
    ),
    cluster_rows = FALSE,
    cluster_cols = FALSE,
    ...
    labels_row = c(
      ...
      paste0(
        ...
        '/',
        original_dscript_sizes['richoux-regular'],
        ')'
      ),
      paste0(
        'RICHOUX-STRICT (',
        ...
        '/',
        original_dscript_sizes['richoux-strict'],
        ')'
      ),
      paste0(
        'D-SCRIPT UNBALANCED (',
        ...
        '/',
        original_dscript_sizes['dscript'],
        ')'
      ),
      #rewired
      ...
      paste0(
        ...
        '/',
        rewired_dscript_sizes['richoux-regular'],
        ')'
      ),
      paste0(
        'RICHOUX-STRICT (',
        ...
        '/',
        rewired_dscript_sizes['richoux-strict'],
        ')'
      ),
      paste0(
        'D-SCRIPT UNBALANCED (',
      ...
      paste0(
        ...
        '/',
        partition_dscript_sizes['huang both'],
        ')'
      ),
      ...
      paste0(
        ...
        '/',
        partition_dscript_sizes['richoux both'],
        ')'
      ),
      paste0(
        'D-SCRIPT UNBALANCED (',
        ...
        '/',
        partition_dscript_sizes['dscript both'],
        ')'
      ),
      #partition both -> 0
      paste0(
      ...
      paste0(
        ...
        '/',
        partition_dscript_sizes['huang 0'],
        ')'
      ),
      ...
      paste0(
        ...
        '/',
        partition_dscript_sizes['richoux 0'],
        ')'
      ),
      paste0(
        'D-SCRIPT UNBALANCED (',
        ...
        '/',
        partition_dscript_sizes['dscript 0'],
        ')'
      )
    ),
    labels_col = c(
    ...
    pheatmap(
      ...
      labels_row = c(
        ...
        paste0(
          ...
          '/',
          partition_dscript_sizes['guo both'],
          ')'
        ),
        paste0(
          'DU (',
          ...
          '/',
          partition_dscript_sizes['du both'],
          ')'
        ),
        paste0(
          'PAN (',
          ...
          '/',
          partition_dscript_sizes['pan both'],
          ')'
        ),
        paste0(
          'RICHOUX-UNIPROT (',
