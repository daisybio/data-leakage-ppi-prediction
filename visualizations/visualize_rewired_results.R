...
dscript_res <-
  '../algorithms/D-SCRIPT-main/results_dscript/rewired/'
...
custom_results <-
  lapply(paste0(
    custom_res,
    list.files(custom_res, pattern = '^rewired_(du|guo|huang|pan|richoux|dscript).*.csv')
  ), fread)
file_names <-
  tstrsplit(
    list.files(custom_res, pattern = '^rewired_(du|guo|huang|pan|richoux|dscript).*.csv'),
...
custom_results[, c('dataset', 'encoding', 'model') := tstrsplit(filename, '_', keep =
                                                                  c(2, 3, 4))]
  ...
  if (measure == 'Accuracy') {
    wide_df <-
      dcast(custom_results[dataset == 'dscript',],
            filename + dataset + encoding + model ~ V1,
            value.var = 'V2')
...
all_results <-
  rbind(all_results, custom_results[, c('Model', 'Dataset', measure), with = FALSE])
...
if(es){
  deepFE_results <-
    lapply(paste0(
      deepFE_res,
      list.files(deepFE_res, pattern = '^rewired_scores_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript)_es.csv', recursive = TRUE)
    ), fread)
  file_names <-
    tstrsplit(
      list.files(deepFE_res, pattern = '^rewired_scores_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript)_es.csv', recursive = TRUE),
  ...
  deepFE_results <-
    lapply(paste0(
      deepFE_res,
      list.files(deepFE_res, pattern = '^rewired_scores_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript).csv', recursive = TRUE)
    ), fread)
  file_names <-
    tstrsplit(
      list.files(deepFE_res, pattern = '^rewired_scores_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript).csv', recursive = TRUE),
...
all_results <-
  rbind(all_results, deepFE_results[, c('Model', 'Dataset', measure), with = FALSE])
...
if(es){
  deepPPI_results <-
    lapply(paste0(
      deepPPI_res,
      list.files(deepPPI_res, pattern = '(FC|LSTM)_rewired_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript)_es.csv')
    ), fread)
  file_names <-
    tstrsplit(
      list.files(deepPPI_res, pattern = '(FC|LSTM)_rewired_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript)_es.csv'),
  ...
  deepPPI_results <-
    lapply(paste0(
      deepPPI_res,
      list.files(deepPPI_res, pattern = '(FC|LSTM)_rewired_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript).csv')
    ), fread)
  file_names <-
    tstrsplit(
      list.files(deepPPI_res, pattern = '(FC|LSTM)_rewired_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript).csv'),
  ...
  balanced_accuracy_FC <-
    0.5 * (deepPPI_results[Model == 'FC' &
                             ...
                             variable == 'Recall', value] + deepPPI_results[Model == 'FC' &
                                                                              Dataset == 'dscript' & variable == 'Specificity', value])
...
n_train <-
  unique(deepPPI_results[variable == 'n_train', c('Dataset', 'variable', 'value')])
...
all_results <-
  rbind(all_results, deepPPI_results[, c('Model', 'Dataset', measure), with = FALSE])
...
if(es){
  pipr_results <-
    lapply(paste0(
      seqppi_res,
      list.files(seqppi_res, pattern = '^rewired_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript)_es.csv')
    ), fread)
  file_names <-
    tstrsplit(
      list.files(seqppi_res, pattern = '^rewired_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript)_es.csv'),
  ...
  pipr_results <-
    lapply(paste0(
      seqppi_res,
      list.files(seqppi_res, pattern = '^rewired_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript).csv')
    ), fread)
  file_names <-
    tstrsplit(
      list.files(seqppi_res, pattern = '^rewired_(du|guo|huang|pan|richoux_regular|richoux_strict|dscript).csv'),
...
pipr_results[, Dataset := tstrsplit(Filename, '_', keep = 2)]
if (measure == 'Accuracy') {
  balanced_accuracy <-
...
colnames(pipr_results) <-
  c('Filename', 'Measure', measure, 'Dataset', 'Model')

all_results <-
  rbind(all_results, pipr_results[, c('Model', 'Dataset', measure), with = FALSE])
...
sprint_results$Dataset[grepl('richoux', sprint_results$Dataset, fixed =
  ...
  gsub('richoux_*', 'richoux-', sprint_results$Dataset[grepl('richoux', sprint_results$Dataset, fixed =
                                                               TRUE)])
all_results <-
  rbind(all_results, sprint_results[, c('Model', 'Dataset', measure), with = FALSE])
...
if(es){
  fwrite(all_results, file = paste0('results/rewired_', measure, '_es.csv'))
}else{
